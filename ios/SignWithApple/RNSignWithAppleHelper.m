...
@interface RNSignWithAppleHelper()<ASAuthorizationControllerDelegate,ASAuthorizationControllerPresentationContextProviding>
@property(nonatomic,strong)Success success;
@property(nonatomic,strong)Failure failure;
@end
@implementation RNSignWithAppleHelper
+ (RNSignWithAppleHelper *)defaultSignInWithAppleModel{
    dispatch_once(&onceToken, ^{
        _instance = [[RNSignWithAppleHelper alloc]init];
    });
    return _instance;
}
//销毁
...
+ (void)signInWithAppleWithButtonRect:(CGRect)rect withSupView:(UIView *)superView withType:(ASAuthorizationAppleIDButtonType)type withStyle:(ASAuthorizationAppleIDButtonStyle)style success:(void(^)(ASAuthorization *authorization,NSString *user))success
    ...
    if (@available(iOS 13.0, *)) {
        RNSignWithAppleHelper *hsiam = [RNSignWithAppleHelper defaultSignInWithAppleModel];
