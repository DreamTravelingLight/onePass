...
import com.ali.onepass.uitls.AppUtils;
import com.ali.onepass.uitls.Constant;
...
public abstract class BaseUIConfig {
    ...
    public String mOperatorName;//运营商名称
    public ReadableMap mConfig;
    public PhoneNumberAuthHelper mAuthHelper;
    public int mScreenWidthDp;
    public int mScreenHeightDp;

    public static BaseUIConfig init(int type, Activity activity, PhoneNumberAuthHelper authHelper, String operatorName,ReadableMap config) {
        switch (type) {
            case Constant.FULL_PORT:
                return new FullPortConfig(activity, authHelper,operatorName,config);
            case Constant.FULL_LAND:
                return new FullLandConfig(activity, authHelper,operatorName,config);
            case Constant.DIALOG_PORT:
                return new DialogPortConfig(activity, authHelper,operatorName,config);
            case Constant.DIALOG_LAND:
//                return new DialogLandConfig(activity, authHelper);
            case Constant.DIALOG_BOTTOM:
                return new DialogBottomConfig(activity, authHelper,operatorName,config);
            case Constant.CUSTOM_VIEW:
                return new CustomViewConfig(activity, authHelper,operatorName,config);
            case Constant.CUSTOM_XML:
                return new CustomXmlConfig(activity, authHelper,operatorName,config);
...
//                return new CustomPicConfig(activity, authHelper);
    ...
    public BaseUIConfig(Activity activity, PhoneNumberAuthHelper authHelper, String operatorName, ReadableMap config) {
        mActivity = activity;
